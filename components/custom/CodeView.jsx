...
function CodeView() {
    ...
    const GenerateAiCode = useCallback(async () => {
        ...
        // Include existing files as context so the AI knows what to modify
    ...
    }, [messages, files, id, UpdateFiles, preprocessFiles]);

    // Auto-trigger on every user message (modifies existing code instead of regenerating)
    useEffect(() => {
        ...
        <div className='relative bg-[#1a1a2e] rounded-xl border border-gray-800 overflow-hidden'>
            ...
            {loading && (
                <div className='p-10 bg-gray-900/80 backdrop-blur-sm absolute top-0 rounded-lg w-full h-full flex flex-col items-center justify-center gap-3'>
                    ...
                    <h2 className='text-white text-lg font-medium'>Updating your project...</h2>
                    <p className='text-gray-400 text-sm'>Applying changes based on your request</p>
                </div>
            )}
        </div>
    );
